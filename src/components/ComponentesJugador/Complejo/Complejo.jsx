...
const Complejo = () => {
    ...
    useEffect(() => {
        //Esta función trae los datos de un complejo, segun el id_complejo del use params
        //O sea, toma el paramtro 1 de la url, que es el id_complejo y la envia al back
        async function fetchComplejo(id_complejo) {
            axios
                .get(`http://localhost:3001/complejo/${id_complejo}`)
                .then((res) => {
        ...
        async function fetchCanchas(id_complejo) {
            //Funcion para traer un json con las canchas segun el id_complejo
            axios
                .get(`http://localhost:3001/complejo/canchas/${id_complejo}`)
                .then((res) => {
        ...
        async function fetchEquiposDelJugador(id_jugador) {
            //Funcion para traer un json con los equipos creados por un jugador
            const jsonDataEqipos = await axios.get(
                `http://localhost:3001/equipo/mis_equipos/${id_jugador}`
            );
    ...
    const fetchFechas = async (fecha) => {
        const jsonDataFechas = await axios.get(
            `http://localhost:3001/complejo/agenda/turnos/${id_complejo}/${fecha}`
        );
        ...
        const fechaSeleccionadaValida =
            fecha === jsonDataFechas.data.fecha_seleccionada;
    ...
    const renderCell = (hora, cancha) => {
        const disponibles =
            fechas.horario_disponibilidad[hora]?.disponibles || [];
        const ocupadas = fechas.horario_disponibilidad[hora]?.ocupadas || [];

        const indiceDisponible = disponibles.findIndex(
            (item) => item.id_cancha === cancha.id_cancha
        );
        ...
        if (disponibles.length > 0 && indiceDisponible !== -1) {
            ...
            ocupadas.some((item) => item.id_cancha === cancha.id_cancha)
        ) {
    ...
    return (
        <div className='ComplejoContainer main'>
            <div className='complejoHeader'>
                ...
                <p className='text-left complejoDireccion'>
                ...
                <div className='centradoDeComplejoDatos'>
                    <div className='complejoDatos text-left'>
                        <div className='complejoAdministrador'>
                            ...
                            <p className='m-0'>
                                Administrador iniciante: lleva 3 meses usasndo
                                CanchApp
                            </p>
                        </div>
                        ...
                        <div className='complejoDescripcion'>
                            ...
                            <p className='m-0'>
                                ...
                                adipisicing elit. Quidem a, inventore ullam
                                animi dolores similique error omnis iusto
                                sapiente? Vero, alias in consequuntur ipsa autem
                                facilis quas veniam dolorem nobis sunt,
                                architecto asperiores distinctio enim quidem
                                totam officia? Sit, in ducimus. Quibusdam omnis
                                amet quia a iusto ipsum libero ipsa.
                            </p>
                        </div>

                        <div className='complejoInfo'>
                            <p className='m-0'>
                                <strong>Teléfono:</strong> {complejo.telefono}
                            </p>
                            <p className='m-0'>
                                <strong>Horarios del club:</strong> horarios
                            </p>
                        </div>
                    </div>
                </div>
